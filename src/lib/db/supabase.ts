...
export const getNews = async (): Promise<NewsPostWithId[]> => {
    ...
    const newsData = (data || []).map(fromSupabaseNews);
...
export const getPaginatedNews = async (limitNum = 20, lastDoc?: any): Promise<{ data: NewsPostWithId[], lastDoc?: any }> => {
    ...
    return { data: newsData, lastDoc: nextLastDoc };
};
...
const generateNewsDataFromEvent = (
    eventData: Partial<Omit<CalendarEvent, 'id' | 'attachments'>>, 
    calendarEventId?: string
): Omit<NewsPost, 'id' | 'slug' | 'attachments'> => {
    ...
    const formattedTime = !eventData.allDay ? eventDate.toLocaleTimeString('en-GB', { 
        hour: '2-digit', 
        minute: '2-digit' 
    }) : null;

    // Create clean, professional news content using the event title and description
    const body_en = `
        <div class="event-announcement">
            <p><strong>📅 ${formattedDate}</strong>${formattedTime ? ` at ${formattedTime}` : ' (All Day)'}</p>
            
            ${eventData.relevantTo && eventData.relevantTo.length > 0 ? 
                `<p><strong>Relevant to:</strong> ${eventData.relevantTo.join(', ')}</p>` : ''}
            
            ${eventData.description_en ? `<p>${eventData.description_en}</p>` : ''}
            
            ${eventData.attachmentUrl ? 
                `<p><strong>📎 Additional Information:</strong> Please see the attached document for more details.</p>` : ''}
        </div>
    `;

    const body_cy = `
        <div class="event-announcement">
            <p><strong>📅 ${formattedDate}</strong>${formattedTime ? ` am ${formattedTime}` : ' (Drwy\'r Dydd)'}</p>
            
            ${eventData.relevantTo && eventData.relevantTo.length > 0 ? 
                `<p><strong>Yn berthnasol i:</strong> ${eventData.relevantTo.join(', ')}</p>` : ''}
            
            ${eventData.description_cy || eventData.description_en ? `<p>${eventData.description_cy || eventData.description_en}</p>` : ''}
            
            ${eventData.attachmentUrl ? 
                `<p><strong>📎 Gwybodaeth Ychwanegol:</strong> Gweler y ddogfen atodedig am fwy o fanylion.</p>` : ''}
        </div>
    `;
    
    return {
        title_en: eventData.title_en || 'Event Announcement',
        title_cy: eventData.title_cy || eventData.title_en || 'Cyhoeddiad Digwyddiad',
        body_en: body_en,
        body_cy: body_cy,
...
export const getCalendarEvents = async (): Promise<CalendarEventWithId[]> => {
    ...
    const eventsData = data.map(fromSupabaseCalendarEvent);
...
export const addCalendarEvent = async (eventData: Omit<CalendarEvent, 'id' | 'attachments'>, crossPost: boolean) => {
    ...
    if (crossPost) {
        // First, create the calendar event to get its ID
        const { data: eventResult, error: eventError } = await supabase
            .from('calendar_events')
            .insert([toSupabaseCalendarEvent(eventData)])
            .select('id')
            .single();
        ...
        // Create the news post with the calendar event ID for bidirectional linking
        const newsData = generateNewsDataFromEvent(eventData, calendarEventId);
        const newsId = await addNews(newsData);
        
        // Update the calendar event with the news post ID
        const { error: updateError } = await supabase
            .from('calendar_events')
            .update({ linked_news_post_id: newsId })
            .eq('id', calendarEventId);
            
        if (updateError) throw updateError;
    } else {
        // Simple insert without cross-posting
        const { error } = await supabase.from('calendar_events').insert([toSupabaseCalendarEvent(eventData)]);
        if (error) throw error;
    }
};
...
export const updateCalendarEvent = async (id: string, eventData: Partial<Omit<CalendarEvent, 'id' | 'attachments'>>, crossPost: boolean) => {
    ...
    if (crossPost) {
        const newsData = generateNewsDataFromEvent(eventData as Omit<CalendarEvent, 'id' | 'attachments'>, id);
        if (finalEventData.linkedNewsPostId) {
            // Update existing linked news post with bidirectional link
            await updateNews(finalEventData.linkedNewsPostId, { ...newsData, linkedCalendarEventId: id });
        } else {
...
export const getPaginatedCalendarEvents = async (limitNum = 20, lastDoc?: any): Promise<{ data: CalendarEventWithId[], lastDoc?: any }> => {
    ...
    // Return all events for browsing; homepage applies lifecycle filtering
    const nextLastDoc = data && data.length === limitNum ? { page } : undefined;
    return { data: eventsData, lastDoc: nextLastDoc };
};
