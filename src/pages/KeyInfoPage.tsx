...
import { Calendar, Shirt, UtensilsCrossed, Download, FileText, Clock, Users, Loader2, ShieldCheck, ExternalLink } from 'lucide-react';
...
const KeyInfoPage: React.FC = () => {
  ...
  useEffect(() => {
    // Mock documents - in real app this would come from Supabase
    setDocuments([
      {
        id: '1',
        title: 'Term Dates 2024-2025',
        file_url: '/documents/term-dates-2024-25.pdf',
        category: 'Term Dates'
      },
      {
        id: '2',
        title: 'School Uniform Policy',
        file_url: '/documents/uniform-policy.pdf',
        category: 'Uniform'
      },
      {
        id: '3',
        title: 'Lunch Menu - Autumn Term',
        file_url: '/documents/lunch-menu-autumn.pdf',
        category: 'Lunch Menu'
      },
      {
        id: '4',
        title: 'Safeguarding Policy',
        file_url: '/documents/safeguarding-policy.pdf',
        category: 'Policy'
      },
      {
        id: '5',
      ...
      },
      {
        ...
        file_url: '/documents/complaints-procedure.pdf',
        category: 'Policy'
      }
    ]);
    setIsLoading(false);
  ...
  const findDocUrl = (category: string) => documents.find(d => d.category === category)?.file_url;
  ...
  const infoCards: InfoCardData[] = [
    ...
    { icon: UtensilsCrossed, title: t.lunchMenu.title, body: t.lunchMenu.body, button: t.lunchMenu.button, category: "Lunch Menu" },
  ];
  
  const policyDocs = documents.filter(d => d.category === 'Policy');
                  ...
                  <div className="flex flex-wrap gap-2">
                    {policyDocs.length > 0 ? policyDocs.map(link => (
                      <Button asChild variant="outline" key={link.id}>
                        <a href={link.file_url} target="_blank" rel="noopener noreferrer">
