...
import { Loader2, Inbox as InboxIcon, Reply, Send, Mail } from 'lucide-react';
...
const ReplyForm = ({ threadId, subject, onSuccess }: { threadId: string; subject: string; onSuccess: () => void }) => {
  ...
  const handleSendReply = async (values: z.infer<typeof replyFormSchema>) => {
    setIsSending(true);
    try {
      // TODO: Replace hardcoded user with real authenticated parent user details
...
export default function InboxPage() {
  ...
  // TODO: Replace with real authenticated parent id
      ...
      if (message.recipient?.id === currentUserId && !message.isReadByParent) {
        try {
          ...
          setThreads((prev) => {
            ...
            const threadToUpdate = newThreads[value].map((m) => (m.id === message.id ? { ...m, isReadByParent: true } : m));
                ...
                <AccordionContent className="bg-muted/50">
                  <div className="p-4 space-y-4">
                    {messages.map((message) => (
                      <div
                        key={message.id}
                        className={cn('p-4 rounded-md flex gap-4', message.sender?.type === 'admin' ? 'bg-card' : 'bg-primary/10')}
                      >
                        ...
                        <div className="flex-grow">
                          <div className="flex justify-between items-baseline">
                            <p className="font-semibold">{message.sender?.type === 'admin' ? message.sender?.name : t.you}</p>
                            <p className="text-xs text-muted-foreground">{format(new Date(message.createdAt), 'dd MMM yyyy, p')}</p>
                          </div>
                          <p className="mt-2 text-sm whitespace-pre-wrap">{message.body}</p>
                        </div>
                      </div>
                    ))}
                    <ReplyForm threadId={threadId} subject={firstMessage.subject} onSuccess={fetchMessages} />
                  </div>
                </AccordionContent>
              </AccordionItem>
            );
